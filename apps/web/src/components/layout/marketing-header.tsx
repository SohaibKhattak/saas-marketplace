        ...
        <nav className="hidden md:flex items-center gap-1">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`px-3.5 py-2 text-sm rounded-sm transition-all duration-200 ${pathname === link.href
        ...
        <div className="flex items-center gap-2">
          {!hasHydrated ? (
            ...
            <Link href={
              ...
              <Button size="lg" className="rounded-sm shadow-sm shadow-primary/20 font-semibold cursor-pointer">Dashboard</Button>
            </Link>
          ) : (
            <>
              <Link href="/login">
          ...
          <nav className="container mx-auto px-4 py-4 flex flex-col gap-1">
            {navLinks.map((link) => (
              <Link
                ...
                className={`px-4 py-2.5 text-sm rounded-sm transition-all ${pathname === link.href
                  ? "text-neutral-900 font-semibold bg-gray-100"
